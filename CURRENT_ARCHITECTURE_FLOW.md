# HFT系统当前架构和调用流程

## 整体架构图

```
┌─────────────────────────────────────────────────────────────────────────┐
│                        Application Layer (Golang)                       │
│  ┌──────────────────────────────────────────────────────────────────┐   │
│  │                      Strategy Engine (未实现)                     │   │
│  │  - PassiveStrategy                                                │   │
│  │  - AggressiveStrategy                                             │   │
│  │  - HedgingStrategy                                                │   │
│  │  - PairwiseArbStrategy                                            │   │
│  └──────────────────────────────────────────────────────────────────┘   │
│                                    ↓ ↑                                   │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │              Indicator Library (✅ 已实现)                       │    │
│  │  - EWMA, OrderImbalance, VWAP, Spread, Volatility               │    │
│  │  - 7个核心指标 + 扩展接口                                         │    │
│  └─────────────────────────────────────────────────────────────────┘    │
│                                    ↓ ↑                                   │
│  ┌──────────────────────────────────────────────────────────────────┐   │
│  │                  ORS Client (✅ 已实现)                           │   │
│  │  - gRPC订单发送                                                   │   │
│  │  - NATS订单回报订阅                                               │   │
│  │  - 测试模式: single/batch/monitor                                │   │
│  └──────────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────────┘
                                    ↓ ↑
                          ┌─────────────────────┐
                          │   NATS Message Bus  │
                          │  (Pub/Sub通信)      │
                          └─────────────────────┘
                                    ↓ ↑
┌─────────────────────────────────────────────────────────────────────────┐
│                         Gateway Layer (C++)                             │
│  ┌───────────────────────────┐          ┌───────────────────────────┐   │
│  │   MD Gateway (未完整)     │          │  ORS Gateway (✅ 已实现)  │   │
│  │  - gRPC服务               │          │  - gRPC服务               │   │
│  │  - NATS发布行情           │          │  - NATS发布订单回报       │   │
│  │  - 共享内存读取           │          │  - 共享内存读写           │   │
│  └───────────────────────────┘          └───────────────────────────┘   │
│            ↓                                      ↓ ↑                    │
│  ┌────────────────────┐              ┌────────────────────────┐         │
│  │  MD Shared Memory  │              │  ORS Shared Memory     │         │
│  │  (SPSC Queue)      │              │  Request/Response      │         │
│  │  [未使用]          │              │  (SPSC Queues)         │         │
│  └────────────────────┘              └────────────────────────┘         │
│                                                ↓ ↑                       │
│                                      ┌───────────────────────────────┐   │
│                                      │ Counter Gateway (✅ 已实现)   │   │
│                                      │  - 读取Request队列            │   │
│                                      │  - 写入Response队列           │   │
│                                      │  - 调用Counter API            │   │
│                                      └───────────────────────────────┘   │
│                                                ↓ ↑                       │
│                                      ┌───────────────────────────────┐   │
│                                      │  Counter API (✅ 已实现)      │   │
│                                      │  - ICounterAPI接口            │   │
│                                      │  - SimulatedCounter实现       │   │
│                                      │  - 未来: EES/CTP柜台          │   │
│                                      └───────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────────┘
                                                ↓ ↑
                                    ┌──────────────────────┐
                                    │  模拟交易所          │
                                    │  - 订单接受 (10ms)   │
                                    │  - 订单成交 (50ms)   │
                                    │  - 成交概率 95%      │
                                    │  - 拒绝概率 2%       │
                                    └──────────────────────┘
```

## 当前已实现的调用流程

### 流程1: 订单发送流程 (完整可用 ✅)

```
[Golang ORS Client]
    │
    │ 1. 构造OrderRequest protobuf
    │    - strategy_id: "test_strategy_1"
    │    - symbol: "ag2412"
    │    - side: BUY/SELL
    │    - price: 7950.0
    │    - quantity: 10
    │
    ↓ gRPC调用 (localhost:50052)
    │
[ORS Gateway - gRPC Service]
    │
    │ 2. 接收gRPC请求
    │    - ORSGatewayImpl::SendOrder()
    │    - 生成order_id: ORD_{timestamp}_{counter}
    │    - 转换为OrderRequestRaw结构
    │
    │ 3. 加入内部队列
    │    - m_pending_requests.push(raw_req)
    │    - 返回gRPC响应 (延迟: ~6μs)
    │
    ↓
    │
[ORS Gateway - Request Writer Thread]
    │
    │ 4. 从内部队列读取
    │    - gateway->GetOrderRequest(&raw_req)
    │    - 写入共享内存Request队列
    │    - req_queue->Push(raw_req)
    │
    ↓ 共享内存 (POSIX shm, lock-free SPSC)
    │
[Counter Gateway - Request Reader Thread]
    │
    │ 5. 从共享内存读取
    │    - req_queue->Pop(raw_req)
    │    - 读取延迟: ~1-2μs
    │
    │ 6. 发送到Counter API
    │    - counter->SendOrder(raw_req, order_id)
    │    - 生成exchange_order_id: EX_{timestamp}_{counter}
    │
    ↓
    │
[Simulated Counter - Process Thread]
    │
    │ 7. 处理订单
    │    - 加入pending_orders队列
    │    - 随机决定接受/拒绝 (2%拒绝概率)
    │
    │ 8a. 订单接受
    │    - 延迟10ms
    │    - 回调: OnOrderAccept()
    │    - status: ACCEPTED
    │
    │ 8b. 订单成交
    │    - 延迟50ms
    │    - 回调: OnOrderFilled()
    │    - status: FILLED
    │    - 成交概率: 95%
    │
    ↓
    │
[Counter Gateway - Callback]
    │
    │ 9. 填充响应数据
    │    - OrderResponseRaw结构
    │    - 填充order_id, status, price, quantity
    │
    │ 10. 写入Response队列
    │    - resp_queue->Push(resp)
    │
    ↓ 共享内存
    │
[ORS Gateway - Response Reader Thread]
    │
    │ 11. 从共享内存读取响应
    │    - resp_queue->Pop(raw_resp)
    │    - 转换为OrderUpdate protobuf
    │
    │ 12. 更新订单簿
    │    - gateway->OnOrderResponse(update)
    │    - 更新内部订单状态
    │
    │ 13. 发布NATS消息
    │    - 主题: "order.{strategy_id}.{symbol}"
    │    - 例如: "order.test_strategy_1.ag2412"
    │
    ↓ NATS消息总线
    │
[Golang ORS Client - NATS Subscriber]
    │
    │ 14. 接收订单更新
    │    - 订阅通配符: "order.test_strategy_1.>"
    │    - 回调: onOrderUpdate()
    │    - 打印订单状态变化
    │
    └─→ 订单流程完成
```

### 流程2: 行情数据流 (部分实现 🚧)

```
[行情源] (未实现)
    │
    ↓ (未来: TCP/UDP接收行情)
    │
[MD Gateway] (框架存在，但未连接)
    │
    │ - 解析原始行情数据
    │ - 写入MD Shared Memory
    │ - 发布到NATS
    │
    ↓ 共享内存 + NATS
    │
[Golang Strategy Engine] (未实现)
    │
    │ - 订阅行情: "md.{symbol}"
    │ - 更新Indicator Library
    │ - 生成交易信号
    │
    └─→ 调用ORS Client发送订单
```

### 流程3: 指标计算流程 (已实现但未集成 ✅)

```
[行情数据 MarketDataUpdate]
    │
    │ protobuf结构:
    │ - symbol: "ag2412"
    │ - exchange: "SHFE"
    │ - bid_price[]: [7949, 7947, ...]
    │ - bid_qty[]: [100, 80, ...]
    │ - ask_price[]: [7951, 7953, ...]
    │ - ask_qty[]: [95, 75, ...]
    │ - last_price: 7950.0
    │
    ↓
    │
[IndicatorLibrary]
    │
    │ UpdateAll(md):
    │ ├─→ EWMA.Update(md)
    │ │   └─→ 计算指数移动平均: 7950.5529
    │ │
    │ ├─→ OrderImbalance.Update(md)
    │ │   └─→ 计算买卖不平衡: 0.0323 (买盘略强)
    │ │
    │ ├─→ VWAP.Update(md)
    │ │   └─→ 计算成交量加权均价: 7949.1325
    │ │
    │ ├─→ Spread.Update(md)
    │ │   └─→ 计算买卖价差: 2.0
    │ │
    │ └─→ Volatility.Update(md)
    │     └─→ 计算波动率: 0.000755
    │
    ↓
    │
[Strategy Engine] (未实现)
    │
    │ 使用指标值生成信号:
    │ - if (order_imbalance > 0.05 && price < vwap):
    │     signal = BUY
    │ - if (volatility > threshold):
    │     reduce_position_size()
    │
    └─→ 生成TradingSignal
```

## 当前测试的完整链路

### 测试场景: 发送单笔订单

```bash
# 终端1: 启动NATS
$ nats-server

# 终端2: 启动ORS Gateway
$ cd /Users/user/PWorks/RD/hft-poc/gateway/build
$ ./ors_gateway
[ORSGateway] Connected to NATS: nats://localhost:4222
[ORSGateway] gRPC Server: 0.0.0.0:50052

# 终端3: 启动Counter Gateway
$ ./counter_gateway
[SimCounter] Connected successfully
[Counter Gateway started successfully]

# 终端4: 发送测试订单
$ cd /Users/user/PWorks/RD/hft-poc/golang
$ ./bin/ors_client -mode single -symbol ag2412

Order Response:
  Order ID:    ORD_1768909036580_000000
  Status:      SUCCESS
  Latency:     105.860583ms

# 日志输出 - ORS Gateway:
[ORSGateway] SendOrder: ORD_1768909036580_000000 symbol=ag2412 side=BUY price=7950 qty=10
[ReqWriter] Written: 1
[RespReader] Read: 1

# 日志输出 - Counter Gateway:
[SimCounter] Order received: EX_1768909036580_000000 symbol=ag2412 side=BUY price=7950 qty=10
[SimCounter] Order accepted: EX_1768909036580_000000
[Callback] Order accepted: EX_1768909036580_000000 (total: 1)
[SimCounter] Order filled: EX_1768909036580_000000 price=7950 qty=10
[Callback] Order filled: EX_1768909036580_000000 (total: 1)

# 客户端收到NATS更新:
[ORSClient] Order Update: ORD_1768909036580_000000 ACCEPTED
[ORSClient] Order Update: ORD_1768909036580_000000 FILLED
```

## 延迟分解

### 订单往返延迟 (~105ms)

```
客户端 → ORS Gateway (gRPC):           ~0.1ms
ORS Gateway 内部处理:                  ~0.006ms
ORS Gateway → 共享内存:                ~0.001ms
Counter Gateway 读取:                  ~0.001ms
Counter Gateway → SimulatedCounter:    ~0.001ms
SimulatedCounter 接受延迟:             10ms      ← 模拟延迟
SimulatedCounter 成交延迟:             50ms      ← 模拟延迟
Counter Gateway 回调:                  ~0.001ms
共享内存 → ORS Gateway:                ~0.001ms
ORS Gateway NATS发布:                  ~0.1ms
NATS → 客户端:                         ~0.1ms
─────────────────────────────────────────────────
总计: ~60.4ms (实际测试: ~105ms，包括其他开销)
```

### 关键路径延迟 (去除模拟延迟)

```
不包含SimulatedCounter的模拟延迟:
客户端 → ORS Gateway → 共享内存 → Counter Gateway →
Counter API → 回调 → 共享内存 → ORS Gateway → 客户端

估算: ~0.5-1ms
实际可达到: <1ms (满足低延迟要求)
```

## 未来集成点

### 1. Strategy Engine集成 (Week 13-14)

```go
type StrategyEngine struct {
    indicators  *indicators.IndicatorLibrary
    orsClient   *client.ORSClient
    strategies  []Strategy
}

func (se *StrategyEngine) OnMarketData(md *mdpb.MarketDataUpdate) {
    // 1. 更新所有指标
    se.indicators.UpdateAll(md)

    // 2. 各策略生成信号
    for _, strategy := range se.strategies {
        signals := strategy.OnMarketData(md, se.indicators)

        // 3. 发送订单
        for _, signal := range signals {
            se.orsClient.SendOrder(ctx, signal.ToOrderRequest())
        }
    }
}
```

### 2. MD Gateway集成 (未来)

```cpp
// MD Gateway读取行情源
void MDGateway::OnMarketDataFromExchange(const char* raw_data) {
    // 1. 解析原始数据
    MarketDataRaw md_raw;
    ParseRawData(raw_data, &md_raw);

    // 2. 写入共享内存
    m_mdQueue->Push(md_raw);

    // 3. 转换为protobuf
    MarketDataUpdate md_update;
    ConvertToProtobuf(md_raw, &md_update);

    // 4. 发布到NATS
    PublishToNATS("md." + md_update.symbol(), md_update);
}
```

## 数据结构对比

### OrderRequestRaw (C++ 共享内存)
```cpp
struct OrderRequestRaw {
    char order_id[64];           // 订单ID
    char symbol[32];             // 合约代码
    uint8_t side;                // 买卖方向
    double price;                // 价格
    int64_t quantity;            // 数量
    uint8_t order_type;          // 订单类型
    uint64_t timestamp;          // 时间戳
};
```

### OrderRequest (protobuf, gRPC/NATS)
```protobuf
message OrderRequest {
    string strategy_id = 1;
    string symbol = 2;
    OrderSide side = 3;
    double price = 4;
    int64 quantity = 5;
    OrderType order_type = 6;
    Exchange exchange = 7;
}
```

## 服务启动顺序

```
正确的启动顺序:

1. NATS Server
   └─→ nats-server

2. ORS Gateway (创建Request和Response共享内存)
   └─→ ./ors_gateway

3. Counter Gateway (连接共享内存)
   └─→ ./counter_gateway

4. Golang客户端/策略引擎
   └─→ ./bin/ors_client
```

## 总结

### ✅ 已完成且可用
1. **完整订单流**: Golang Client → ORS Gateway → Counter Gateway → SimulatedCounter → 回报
2. **共享内存IPC**: SPSC无锁队列，延迟<2μs
3. **NATS消息总线**: Pub/Sub通信，延迟<100μs
4. **指标库**: 7个核心指标，可独立使用

### 🚧 部分实现
1. **MD Gateway**: 框架存在但未连接行情源
2. **行情数据流**: 未实现端到端

### ⏳ 待实现
1. **Strategy Engine**: 核心策略引擎
2. **指标库集成**: 连接行情流和策略
3. **Portfolio Manager**: 组合管理
4. **Risk Manager**: 风控管理

### 🎯 下一步
Week 13-14: 实现核心策略，将指标库集成到完整交易流程中。
