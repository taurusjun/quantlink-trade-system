# HFT-POC 系统启动和全链路测试指南

**创建时间**: 2026-01-20
**系统版本**: v0.1 POC

---

## 📐 系统架构总览

```
┌─────────────────────────────────────────────────────────────────┐
│                         应用层 (Golang)                          │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐         │
│  │ Strategy     │  │  Portfolio   │  │     Risk     │         │
│  │ Engine       │  │  Manager     │  │   Manager    │         │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘         │
│         │                  │                  │                  │
│         └──────────────────┴──────────────────┘                 │
│                             │                                     │
│         ┌───────────────────┴───────────────────┐               │
│         ▼                                       ▼               │
│  ┌──────────────┐                       ┌──────────────┐       │
│  │  MD Client   │◄──────────────────────┤ ORS Client   │       │
│  │  (行情订阅)   │                       │  (订单发送)   │       │
│  └──────┬───────┘                       └──────┬───────┘       │
│         │                                       │                │
└─────────┼───────────────────────────────────────┼───────────────┘
          │ gRPC/NATS                             │ gRPC
          │ (行情推送)                             │ (订单/回报)
┌─────────┼───────────────────────────────────────┼───────────────┐
│         │                Gateway层 (C++)         │                │
│         ▼                                       ▼                │
│  ┌──────────────┐                       ┌──────────────┐       │
│  │  MD Gateway  │                       │ ORS Gateway  │       │
│  │  (行情网关)   │                       │  (订单网关)   │       │
│  └──────▲───────┘                       └──────┬───────┘       │
│         │                                       │                │
│         │ Shared Memory                         │ Shared Memory  │
│         │ (SPSC Queue)                          ▼                │
│         │                               ┌──────────────┐        │
│  ┌──────┴───────┐                      │   Counter    │        │
│  │     MD       │                      │   Gateway    │        │
│  │  Simulator   │                      │ (交易网关)    │        │
│  └──────────────┘                      └──────┬───────┘        │
│                                                │                 │
└────────────────────────────────────────────────┼────────────────┘
                                                 │
                                                 ▼
                                        ┌────────────────┐
                                        │    Exchange    │
                                        │   (模拟/真实)   │
                                        └────────────────┘
```

---

## 🎯 核心组件说明

### Gateway层 (C++)

#### 1. MD Gateway (行情网关)
**位置**: `gateway/build/md_gateway`

**功能**:
- 从共享内存队列读取原始行情数据
- 转换为Protobuf格式 (MarketDataUpdate)
- 通过gRPC Streaming推送给订阅者
- 通过NATS发布行情消息 (可选)

**端口**:
- gRPC: `50051` (默认)
- 共享内存: `/tmp/hft_md_queue` 或 `/dev/shm/hft_md_queue`

**启动命令**:
```bash
cd gateway/build
./md_gateway [queue_name]
```

---

#### 2. ORS Gateway (订单路由网关)
**位置**: `gateway/build/ors_gateway`

**功能**:
- 提供gRPC服务接收订单请求
- 将订单转换为Raw格式写入共享内存
- 从共享内存读取订单回报
- 转换为Protobuf推送给客户端

**端口**:
- gRPC: `50052` (默认)
- 请求队列: `/tmp/ors_request`
- 响应队列: `/tmp/ors_response`

**启动命令**:
```bash
cd gateway/build
./ors_gateway -a 0.0.0.0:50052 -r ors_request -s ors_response
```

**参数说明**:
- `-a, --address`: gRPC监听地址
- `-r, --req-queue`: 请求队列名称
- `-s, --resp-queue`: 响应队列名称
- `-n, --nats`: NATS服务器地址 (可选)

---

#### 3. Counter Gateway (交易所网关)
**位置**: `gateway/build/counter_gateway`

**功能**:
- 从共享内存读取订单请求
- 调用交易所API发送订单 (当前使用模拟API)
- 接收交易所回报
- 写入共享内存响应队列

**端口**:
- 请求队列: `/tmp/ors_request`
- 响应队列: `/tmp/ors_response`

**启动命令**:
```bash
cd gateway/build
./counter_gateway -r ors_request -s ors_response
```

**参数说明**:
- `-r, --req-queue`: 请求队列名称
- `-s, --resp-queue`: 响应队列名称
- `-t, --type`: Counter类型 (simulated/ees/ctp)

**当前状态**: 使用 `SimulatedCounter`，自动接受所有订单并模拟成交

---

#### 4. MD Simulator (行情模拟器)
**位置**: `gateway/build/md_simulator`

**功能**:
- 生成模拟行情数据
- 写入共享内存队列
- 支持多品种、可配置频率

**启动命令**:
```bash
cd gateway/build
./md_simulator <frequency_hz> [queue_name]

# 示例：1000Hz行情
./md_simulator 1000 queue
```

**参数说明**:
- `frequency_hz`: 行情频率 (Hz)，如 100, 1000, 10000
- `queue_name`: 共享内存队列名称 (可选，默认: "queue")

---

### 应用层 (Golang)

#### 5. MD Client (行情客户端)
**位置**: `golang/cmd/md_client/main.go`

**功能**:
- 连接MD Gateway
- 订阅指定品种行情
- 统计延迟和吞吐量

**启动命令**:
```bash
cd golang/cmd/md_client
go run main.go -gateway localhost:50051 -symbols ag2412,cu2412

# 或使用NATS
go run main.go -nats -nats-url nats://localhost:4222
```

**参数说明**:
- `-gateway`: MD Gateway地址
- `-symbols`: 订阅品种列表 (逗号分隔)
- `-nats`: 使用NATS订阅
- `-nats-url`: NATS服务器地址

---

#### 6. ORS Client (订单客户端)
**位置**: `golang/cmd/ors_client/main.go`

**功能**:
- 连接ORS Gateway
- 发送测试订单
- 接收订单回报

**启动命令**:
```bash
cd golang/cmd/ors_client
go run main.go -gateway localhost:50052
```

---

#### 7. Strategy Demo (策略演示)
**位置**: `golang/cmd/strategy_demo/main.go`

**功能**:
- 演示单个策略运行
- 展示策略引擎基本功能

**启动命令**:
```bash
cd golang/cmd/strategy_demo
go run main.go
```

**注意**: 当前有编译错误，需要修复

---

#### 8. Integrated Demo (集成演示)
**位置**: `golang/cmd/integrated_demo/main.go`

**功能**:
- 完整演示策略引擎 + 组合管理 + 风控
- 运行多个策略
- 展示系统集成效果

**启动命令**:
```bash
cd golang/cmd/integrated_demo
go run main.go
```

**注意**: 当前有编译错误，需要修复

---

#### 9. All Strategies Demo (所有策略演示)
**位置**: `golang/cmd/all_strategies_demo/main.go`

**功能**:
- 演示所有5种策略
- 展示策略多样性

**启动命令**:
```bash
cd golang/cmd/all_strategies_demo
go run main.go
```

**注意**: 当前有编译错误，需要修复

---

## 🔄 数据流向详解

### 行情数据流 (Market Data Flow)

```
1. MD Simulator 生成行情
   ↓ (写入共享内存)
2. 共享内存队列: /tmp/hft_md_queue
   ↓ (MD Gateway读取)
3. MD Gateway
   ├─→ gRPC Streaming → MD Client → Strategy Engine
   └─→ NATS Publish → NATS Subscriber → Strategy Engine
```

**关键特性**:
- **高性能**: 共享内存零拷贝传输
- **低延迟**: < 10μs 网关处理延迟
- **可靠性**: SPSC无锁队列保证顺序

---

### 订单数据流 (Order Flow)

```
1. Strategy Engine 生成交易信号
   ↓
2. ORS Client 发送订单请求
   ↓ (gRPC调用)
3. ORS Gateway 接收请求
   ↓ (写入共享内存)
4. 共享内存: /tmp/ors_request
   ↓ (Counter Gateway读取)
5. Counter Gateway
   ↓ (调用交易所API)
6. Exchange (模拟/真实)
   ↓ (回报)
7. Counter Gateway 接收回报
   ↓ (写入共享内存)
8. 共享内存: /tmp/ors_response
   ↓ (ORS Gateway读取)
9. ORS Gateway
   ↓ (gRPC推送)
10. ORS Client
    ↓
11. Strategy Engine 更新持仓和PNL
```

**关键特性**:
- **解耦**: Gateway层与业务层分离
- **灵活**: 支持多种交易所API
- **安全**: 风控在Strategy层和Gateway层双重检查

---

## 🚀 快速启动流程

### 方式1: 最小化启动 (仅Gateway层测试)

**适用场景**: 测试Gateway性能，不涉及策略

```bash
# 终端1: 启动MD Simulator
cd /Users/user/PWorks/RD/quantlink-trade-system/gateway/build
./md_simulator 1000 queue

# 终端2: 启动MD Gateway
./md_gateway queue

# 终端3: 测试行情接收
cd /Users/user/PWorks/RD/quantlink-trade-system/golang/cmd/md_client
go run main.go -gateway localhost:50051 -symbols ag2412
```

**验证**: 终端3应该显示持续的行情更新和延迟统计

---

### 方式2: ORS链路测试 (订单路由测试)

**适用场景**: 测试订单路由，不涉及策略

```bash
# 终端1: 启动Counter Gateway
cd /Users/user/PWorks/RD/quantlink-trade-system/gateway/build
./counter_gateway -r ors_request -s ors_response

# 终端2: 启动ORS Gateway
./ors_gateway -a 0.0.0.0:50052 -r ors_request -s ors_response

# 终端3: 测试订单发送
cd /Users/user/PWorks/RD/quantlink-trade-system/golang/cmd/ors_client
go run main.go -gateway localhost:50052
```

**验证**:
- 终端1应显示接收到订单并回报
- 终端3应显示订单状态更新

---

### 方式3: 指标演示 (无网络依赖)

**适用场景**: 演示指标计算，不需要Gateway

```bash
cd /Users/user/PWorks/RD/quantlink-trade-system/golang/cmd/indicator_demo
go run main.go
```

**验证**: 应显示10个指标的计算结果和性能统计

---

## 🧪 全链路测试 (End-to-End Test)

### 测试目标
验证从行情接收 → 策略决策 → 订单发送 → 回报处理的完整流程

---

### 准备工作

#### 1. 检查依赖
```bash
# 检查NATS (可选，如果使用NATS订阅)
which nats-server
# 如果没有: brew install nats-server

# 检查Go
go version  # 需要 >= 1.19

# 检查Gateway可执行文件
ls -la /Users/user/PWorks/RD/quantlink-trade-system/gateway/build/
# 应该看到: md_simulator, md_gateway, ors_gateway, counter_gateway
```

#### 2. 清理环境
```bash
# 清理共享内存
rm -f /tmp/hft_* /dev/shm/hft_* 2>/dev/null

# 停止旧进程
pkill -f "md_gateway"
pkill -f "ors_gateway"
pkill -f "counter_gateway"
pkill -f "md_simulator"
pkill -f "nats-server"
```

---

### 启动顺序 (重要！)

#### Step 1: 启动NATS (可选)
```bash
# 终端1
nats-server --port 4222

# 验证
nats-cli ping
```

---

#### Step 2: 启动Counter Gateway (交易所侧)
```bash
# 终端2
cd /Users/user/PWorks/RD/quantlink-trade-system/gateway/build
./counter_gateway -r ors_request -s ors_response

# 应该看到:
# ╔═══════════════════════════════════════════════════════════╗
# ║             HFT Counter Gateway (Simulated)              ║
# ║                  Connecting to Exchange                   ║
# ╚═══════════════════════════════════════════════════════════╝
# [Main] Request queue opened: ors_request
# [Main] Response queue opened: ors_response
# [Counter] Initialized (type: Simulated)
```

**关键点**: Counter Gateway必须先启动，创建共享内存队列

---

#### Step 3: 启动ORS Gateway (订单网关)
```bash
# 终端3
cd /Users/user/PWorks/RD/quantlink-trade-system/gateway/build
./ors_gateway -a 0.0.0.0:50052 -r ors_request -s ors_response

# 应该看到:
# ╔═══════════════════════════════════════════════════════════╗
# ║         HFT Order Routing Service (ORS) Gateway          ║
# ║                   Shared Memory Mode                      ║
# ╚═══════════════════════════════════════════════════════════╝
# [Main] gRPC server listening on 0.0.0.0:50052
# [ResponseReader] Started
```

**关键点**: ORS Gateway连接共享内存队列，提供gRPC服务

---

#### Step 4: 启动MD Simulator (行情源)
```bash
# 终端4
cd /Users/user/PWorks/RD/quantlink-trade-system/gateway/build
./md_simulator 100 queue  # 100Hz行情，适合测试

# 应该看到:
# Market Data Simulator
# Frequency: 100 Hz
# Queue: queue
# Press Ctrl+C to stop
#
# [Simulator] Pushed 1000 ticks to queue...
```

**关键点**:
- 频率不要太高，100-1000Hz适合测试
- 队列名称要与MD Gateway匹配

---

#### Step 5: 启动MD Gateway (行情网关)
```bash
# 终端5
cd /Users/user/PWorks/RD/quantlink-trade-system/gateway/build
./md_gateway queue  # 队列名称与simulator匹配

# 应该看到:
# [Main] MD Gateway starting...
# [Main] Shared memory queue opened: queue
# [Main] gRPC server listening on 0.0.0.0:50051
# [Reader] Processed 1000 messages, avg latency: 8.5 μs
```

**关键点**:
- 队列名称必须与MD Simulator匹配
- 观察延迟统计，应该在10μs以内

---

#### Step 6: 启动策略引擎 (业务层)
```bash
# 终端6
cd /Users/user/PWorks/RD/quantlink-trade-system/golang/cmd/integrated_demo

# 如果有编译错误，先修复
# 然后运行
go run main.go

# 应该看到:
# ╔═══════════════════════════════════════════════════════════╗
# ║         HFT Integrated System Demo                       ║
# ║    Strategy Engine + Portfolio + Risk Management         ║
# ╚═══════════════════════════════════════════════════════════╝
#
# [Main] Creating Risk Manager...
# [Main] ✓ Risk Manager started
# [Main] Creating Portfolio Manager...
# [Main] ✓ Portfolio Manager started
# [Main] Creating Strategy Engine...
# [Main] Warning: Failed to connect to ORS Gateway (may continue in demo mode)
# [Main] Creating strategies...
```

---

### 验证检查清单

#### ✅ Gateway层验证

**MD Gateway检查**:
```bash
# 检查进程
ps aux | grep md_gateway

# 检查共享内存
ls -lh /tmp/hft_md_* /dev/shm/hft_md_*

# 检查gRPC端口
lsof -i :50051

# 查看日志统计
tail -f /tmp/gateway.log  # 如果有日志文件
```

**ORS Gateway检查**:
```bash
# 检查进程
ps aux | grep ors_gateway

# 检查gRPC端口
lsof -i :50052

# 检查共享内存队列
ls -lh /tmp/ors_*
```

**Counter Gateway检查**:
```bash
# 检查进程
ps aux | grep counter_gateway

# 查看订单处理日志
# 终端2应该显示: [Callback] Order accepted/filled
```

---

#### ✅ 行情链路验证

**测试行情接收**:
```bash
# 新终端
cd /Users/user/PWorks/RD/quantlink-trade-system/golang/cmd/md_client
go run main.go -gateway localhost:50051 -symbols ag2412

# 应该看到:
# [Client] Connected to gateway: localhost:50051
# [Client] Subscribed to symbols: [ag2412]
# [Client] Received: ag2412 @ 5321.50 | Latency: 125μs
# [Stats] 1000 ticks | Avg: 120μs | P99: 250μs | Rate: 995/s
```

**预期指标**:
- ✅ 延迟 < 500μs (本地测试)
- ✅ 吞吐量接近模拟器频率
- ✅ 无消息丢失

---

#### ✅ 订单链路验证

**测试订单发送**:
```bash
# 新终端
cd /Users/user/PWorks/RD/quantlink-trade-system/golang/cmd/ors_client
go run main.go -gateway localhost:50052

# 应该看到:
# [Client] Connected to ORS Gateway: localhost:50052
# [Client] Sending test order...
# [Client] Order sent: order_12345
# [Client] Update: order_12345 -> ACCEPTED
# [Client] Update: order_12345 -> FILLED (100 @ 5321.50)
```

**预期指标**:
- ✅ 订单延迟 < 1ms
- ✅ 状态: NEW → ACCEPTED → FILLED
- ✅ 无拒单 (模拟模式下)

---

#### ✅ 策略引擎验证

**检查策略状态**:
```bash
# 在integrated_demo终端观察
# 应该看到:
# [Strategy:passive_ag] Generated signal: BUY 10 @ 5321.25
# [Strategy:aggressive_cu] Generated signal: BUY 20 @ 7654.00
# [RiskManager] Checking limits...
# [Portfolio] Updated position: ag2412 +10
```

**预期行为**:
- ✅ 策略接收行情并生成信号
- ✅ 风控检查通过
- ✅ 组合管理更新持仓
- ✅ 订单发送到ORS Gateway

---

### 性能指标参考

| 环节 | 预期延迟 | 备注 |
|------|---------|------|
| **Simulator → SharedMemory** | < 1μs | 内存写入 |
| **SharedMemory → MD Gateway** | < 10μs | 内存读取+转换 |
| **MD Gateway → gRPC Client** | < 100μs | 本地网络 |
| **gRPC Client → Strategy** | < 50μs | 函数调用 |
| **端到端行情延迟** | **< 200μs** | 总延迟 |
| | | |
| **Strategy → ORS Client** | < 50μs | 函数调用 |
| **ORS Client → ORS Gateway** | < 100μs | gRPC调用 |
| **ORS Gateway → SharedMemory** | < 10μs | 内存写入 |
| **SharedMemory → Counter** | < 10μs | 内存读取 |
| **Counter → Exchange(模拟)** | < 50μs | 模拟成交 |
| **端到端订单延迟** | **< 300μs** | 总延迟 |

**注意**: 以上是本地测试的理想值，实盘环境会受网络延迟影响

---

## 🔧 常见问题和解决方案

### 问题1: Gateway无法启动

**症状**:
```
Error: Failed to open shared memory queue
```

**原因**: 共享内存文件权限问题或已存在

**解决**:
```bash
# 清理共享内存
rm -f /tmp/hft_* /dev/shm/hft_*

# 重新启动
```

---

### 问题2: MD Client无法连接

**症状**:
```
Failed to connect to gateway: connection refused
```

**原因**: MD Gateway未启动或端口不对

**解决**:
```bash
# 检查MD Gateway进程
ps aux | grep md_gateway

# 检查端口
lsof -i :50051

# 确认启动顺序: Simulator → Gateway → Client
```

---

### 问题3: 无行情数据

**症状**: MD Client连接成功但收不到行情

**原因**:
- Simulator未启动
- 队列名称不匹配
- Simulator频率为0

**解决**:
```bash
# 检查Simulator进程
ps aux | grep md_simulator

# 检查队列名称
# Simulator: ./md_simulator 100 queue
# Gateway:   ./md_gateway queue
# 必须匹配！

# 重启Simulator并指定合理频率
./md_simulator 100 queue
```

---

### 问题4: 订单无响应

**症状**: 发送订单后无回报

**原因**:
- Counter Gateway未启动
- 共享内存队列不匹配
- ORS Gateway未连接Counter

**解决**:
```bash
# 检查启动顺序 (必须按顺序！)
# 1. Counter Gateway (创建队列)
# 2. ORS Gateway (连接队列)

# 检查队列名称一致
# Counter: ./counter_gateway -r ors_request -s ors_response
# ORS:     ./ors_gateway -r ors_request -s ors_response
```

---

### 问题5: Demo程序编译错误

**症状**:
```
fmt.Println arg list ends with redundant newline
```

**原因**: fmt.Println不需要手动添加换行符

**解决**:
```bash
# 找到类似这样的代码:
fmt.Println("message\n")

# 修改为:
fmt.Println("message")
```

---

### 问题6: 策略无信号

**症状**: Strategy Engine启动但不生成交易信号

**原因**:
- 未收到行情
- 指标未就绪
- 信号阈值太高

**解决**:
```bash
# 1. 检查是否收到行情
# 在Strategy Engine输出中查找: [Strategy] OnMarketData

# 2. 降低信号阈值 (修改配置)
"signal_threshold": 0.1  # 从0.6降到0.1

# 3. 禁用波动率检查
"min_volatility": 0.0
```

---

## 📝 测试脚本

### 自动化测试脚本 (已有)

**MD Gateway + NATS测试**:
```bash
cd /Users/user/PWorks/RD/quantlink-trade-system
./scripts/test_md_gateway_with_nats.sh
```

**功能**:
- 自动启动NATS、Simulator、Gateway
- 运行10秒测试
- 显示统计结果
- 自动清理

---

### 全链路测试脚本 (推荐创建)

创建 `scripts/test_full_chain.sh`:

```bash
#!/bin/bash
set -e

echo "================================================"
echo "  HFT Full Chain Integration Test"
echo "================================================"

PROJECT_ROOT="/Users/user/PWorks/RD/quantlink-trade-system"
BUILD_DIR="$PROJECT_ROOT/gateway/build"

# 清理函数
cleanup() {
    echo "Stopping all services..."
    pkill -f "md_gateway" 2>/dev/null || true
    pkill -f "ors_gateway" 2>/dev/null || true
    pkill -f "counter_gateway" 2>/dev/null || true
    pkill -f "md_simulator" 2>/dev/null || true
    rm -f /tmp/hft_* /dev/shm/hft_* 2>/dev/null || true
    echo "Cleanup done"
}

trap cleanup EXIT INT TERM

# 1. 启动Counter Gateway
echo "[1/5] Starting Counter Gateway..."
cd "$BUILD_DIR"
./counter_gateway -r ors_request -s ors_response > /tmp/counter.log 2>&1 &
COUNTER_PID=$!
sleep 2

# 2. 启动ORS Gateway
echo "[2/5] Starting ORS Gateway..."
./ors_gateway -a 0.0.0.0:50052 -r ors_request -s ors_response > /tmp/ors.log 2>&1 &
ORS_PID=$!
sleep 2

# 3. 启动MD Simulator
echo "[3/5] Starting MD Simulator (100Hz)..."
./md_simulator 100 queue > /tmp/simulator.log 2>&1 &
SIMULATOR_PID=$!
sleep 2

# 4. 启动MD Gateway
echo "[4/5] Starting MD Gateway..."
./md_gateway queue > /tmp/md_gateway.log 2>&1 &
GATEWAY_PID=$!
sleep 3

# 5. 测试行情接收
echo "[5/5] Testing market data reception..."
cd "$PROJECT_ROOT/golang/cmd/md_client"
timeout 10s go run main.go -gateway localhost:50051 -symbols ag2412 > /tmp/md_client.log 2>&1 || true

# 显示结果
echo ""
echo "================================================"
echo "  Test Results"
echo "================================================"
echo "Counter Gateway: PID=$COUNTER_PID"
grep -E "(Order|accepted|filled)" /tmp/counter.log | head -5 || echo "  No orders yet"
echo ""
echo "MD Gateway: PID=$GATEWAY_PID"
grep -E "(Processed|messages)" /tmp/md_gateway.log | tail -3 || echo "  No statistics"
echo ""
echo "MD Client:"
grep -E "(Received|Stats)" /tmp/md_client.log | tail -5 || echo "  No data received"
echo ""
echo "Full logs:"
echo "  Counter:  /tmp/counter.log"
echo "  ORS:      /tmp/ors.log"
echo "  Simulator:/tmp/simulator.log"
echo "  Gateway:  /tmp/md_gateway.log"
echo "  Client:   /tmp/md_client.log"
echo "================================================"
```

---

## 🎯 下一步行动

### 1. 立即可做 (10分钟)

修复Demo程序编译错误:
```bash
cd /Users/user/PWorks/RD/quantlink-trade-system/golang

# 找到所有 fmt.Println("...\n") 并移除 \n
grep -r "fmt.Println.*\\\\n" cmd/

# 或批量修复
find cmd/ -name "*.go" -exec sed -i '' 's/fmt.Println("\(.*\)\\n")/fmt.Println("\1")/g' {} \;
```

---

### 2. 基础测试 (30分钟)

**测试1**: Gateway性能测试
```bash
# 使用已有脚本
./scripts/test_md_gateway_with_nats.sh
```

**测试2**: 手动全链路测试
- 按照上述启动顺序依次启动所有服务
- 验证每个环节的数据流

---

### 3. 创建自动化脚本 (1小时)

创建以下脚本方便日常开发:

1. `scripts/start_all.sh` - 启动所有服务
2. `scripts/stop_all.sh` - 停止所有服务
3. `scripts/test_full_chain.sh` - 全链路测试
4. `scripts/check_status.sh` - 检查所有服务状态

---

## 📊 监控和调试

### 性能监控

**实时监控命令**:
```bash
# 进程监控
watch -n 1 'ps aux | grep -E "(gateway|simulator)" | grep -v grep'

# 共享内存监控
watch -n 1 'ls -lh /tmp/hft_* /dev/shm/hft_* 2>/dev/null'

# 端口监控
watch -n 1 'lsof -i :50051,50052'

# 系统资源
htop  # 查看CPU和内存
```

---

### 日志分析

**关键日志位置**:
```bash
# Gateway日志 (如果重定向)
/tmp/md_gateway.log
/tmp/ors.log
/tmp/counter.log
/tmp/simulator.log

# 策略日志 (stdout)
# 在运行终端查看

# 系统日志
/var/log/system.log  # macOS
```

**分析命令**:
```bash
# 查找错误
grep -i error /tmp/*.log

# 统计吞吐量
grep "Processed" /tmp/md_gateway.log | tail -10

# 查看延迟分布
grep "latency" /tmp/md_client.log
```

---

### 调试技巧

**1. 使用gdb调试C++程序**:
```bash
cd /Users/user/PWorks/RD/quantlink-trade-system/gateway/build
gdb ./md_gateway
(gdb) run queue
(gdb) bt  # 如果崩溃，查看堆栈
```

**2. 使用delve调试Go程序**:
```bash
cd golang/cmd/md_client
dlv debug main.go -- -gateway localhost:50051
(dlv) break main.main
(dlv) continue
```

**3. 网络抓包**:
```bash
# 抓取gRPC流量
sudo tcpdump -i lo0 -nn -X port 50051 or port 50052

# 使用Wireshark
# Filter: tcp.port == 50051 || tcp.port == 50052
```

---

## 📚 参考资料

### 架构文档
- [GATEWAY_IMPLEMENTATION_STATUS.md](GATEWAY_IMPLEMENTATION_STATUS.md) - Gateway实现状态
- [INDICATOR_IMPLEMENTATION_STATUS.md](INDICATOR_IMPLEMENTATION_STATUS.md) - 指标实现状态
- [TEST_FIXES_REPORT.md](golang/TEST_FIXES_REPORT.md) - 测试修复报告
- [P0_INDICATORS_COMPLETION_REPORT.md](golang/pkg/indicators/P0_INDICATORS_COMPLETION_REPORT.md) - P0指标完成报告

### 技术栈
- **语言**: C++17, Go 1.19+
- **通信**: gRPC, NATS, Shared Memory
- **序列化**: Protocol Buffers 3
- **并发**: C++ std::thread, Go goroutines

### 外部依赖
- NATS Server (可选)
- Protocol Buffers编译器
- gRPC库
- CMake (构建Gateway)

---

## ✅ 快速检查清单

部署前检查:

- [ ] 所有Gateway可执行文件已编译
- [ ] 共享内存权限正确 (/tmp 和 /dev/shm)
- [ ] 端口50051和50052未被占用
- [ ] NATS Server已安装 (如需要)
- [ ] Go环境配置正确
- [ ] Demo程序编译错误已修复

启动检查:

- [ ] Counter Gateway先启动
- [ ] ORS Gateway正常连接
- [ ] MD Simulator生成行情
- [ ] MD Gateway接收行情
- [ ] 客户端可连接Gateway
- [ ] 订单可正常发送和回报

运行检查:

- [ ] 行情延迟 < 500μs
- [ ] 订单延迟 < 1ms
- [ ] 无消息丢失
- [ ] 无异常日志
- [ ] 策略正常生成信号
- [ ] 风控正常工作

---

## 🎉 总结

HFT-POC系统采用**分层架构**:
- **Gateway层** (C++): 高性能、低延迟的数据通道
- **应用层** (Golang): 灵活的策略引擎和风控系统

**核心优势**:
- ✅ 共享内存实现微秒级延迟
- ✅ 模块化设计便于扩展
- ✅ 完善的测试覆盖
- ✅ 支持模拟和实盘切换

**当前状态**:
- 核心功能完整 ✅
- 需要修复Demo编译错误 ⚠️
- 需要补充MD Gateway真实数据源 🔄

**下一步**: 按照本文档完成全链路测试，确保系统稳定性。
