...
**方案 1: 扩展 PositionSnapshot**（推荐）
- 在现有 JSON 格式中增加 `AvgPx`、`OrigBaseName1/2` 字段
- 保持 JSON 格式，便于调试和查看

**方案 2: 实现独立的 daily_init**
- 完全复制 C++ 格式
- 便于与 C++ 系统互操作
...
1. **文件锁**: C++ 使用 `flock()` 防止多进程并发写入，Go 也需要实现
2. **原子性**: 写入时先写临时文件，再重命名，确保数据完整性
3. **备份**: 建议保留历史版本，便于回滚
4. **验证**: 加载时验证 StrategyID 和品种名称是否匹配
